...
float delta;
float time1;
byte Digit[] = {0x3f, 0x06, 0x5b, 0x4f, 0x66, 0x6d, 0x7d, 0x07, 0x7f, 0x67, 0x79};
...
void setup() {
        ...
        Wire.begin();
        rtc.begin();
        MsTimer2::set(5000, low_battery); // 7s период
        MsTimer2::start();
...
void low_battery(){
        ...
        for(int i = 0; i < 5; i++) {
                average += analogRead(pin_ADC);
        }
        Serial.println((1.057*(average/5)/1023)/0.09157);
        //Serial.println(scale*(average/5));
        if (scale*(average/5) < 3.20) {
